...
def _best_microphone_index():
    try:
        ...
        # Look for typical "Real" mics first
        priority = ["mic", "realtek", "usb", "high definition", "audio device"]
        for p in priority:
            for i, name in enumerate(names):
                if p in name.lower():
                    print(f"[MIC] Found priority hardware: [{i}] {name}")
                    return i
        # Fallback to the first available non-virtual mic
        skip = {"virtual", "loopback", "stereo mix", "wave out", "what u hear"}
        for i, name in enumerate(names):
            if not any(s in name.lower() for s in skip):
                print(f"[MIC] Using hardware: [{i}] {name}")
                return i
    except Exception:
        pass
    return None
...
def _capture_audio():
    ...
    for attempt in range(1, 4):
        print(f"\n[MIC] [{attempt}/3] Speak in English (listening now)...")
        try:
            mic_kwargs = {} if mic_index is None else {"device_index": mic_index}
            with sr.Microphone(**mic_kwargs) as source:
                # Optimized sensitivity for Windows
